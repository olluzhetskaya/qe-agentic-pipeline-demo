...
const DESIGN_TARGET = 'data/test-design.json';
const SPEC_DIR = path.join(ROOT, 'src/tests');

const AGENTS = new Set(['test-design-reviewer', 'code-reviewer']);
...
export function parseVerdict(file: string, findings: string[]): JudgeVerdict | null {
  ...
  if (!AGENTS.has(agent)) findings.push(`${label}: agent must be test-design-reviewer or code-reviewer`);
  ...
  if (agent === 'test-design-reviewer' && target !== DESIGN_TARGET) {
    findings.push(`${label}: test-design-reviewer target must be ${DESIGN_TARGET}`);
  }
  if (agent === 'code-reviewer' &&
      (!target.startsWith('src/tests/') || !target.endsWith('.spec.ts'))) {
    findings.push(`${label}: code-reviewer target must be under src/tests/ and end in .spec.ts`);
  }
...
export function runRequiredJudgeGates(): string[] {
  const findings: string[] = [];
  const verdicts = loadVerdicts(findings);
  requirePass('test-design-reviewer', DESIGN_TARGET, verdicts, findings);
  for (const file of filesUnder(SPEC_DIR, '.spec.ts')) {
    requirePass('code-reviewer', rel(file).split(path.sep).join('/'), verdicts, findings);
  }
  return findings;
}
