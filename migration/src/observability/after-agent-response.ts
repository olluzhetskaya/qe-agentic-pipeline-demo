...
const total = inputTokens + completion + cacheRead + cacheWrite;

const rates = ratesFor(model);
const inputCost = usd(inputTokens, rates.input);
...
const lf = getLangfuse(convId);
const promptText = takePrompt(payload.generation_id ?? '');
recordAgentTurn(lf, {
  generationId: payload.generation_id ?? null,
  prompt: promptText,
  response: text.length > OUTPUT_PREVIEW ? `${text.slice(0, OUTPUT_PREVIEW)}…` : text,
  model,
  usageDetails: {
    input: inputTokens,
    output: completion,
  ...
  costDetails: {
    ...
    cache_read_input_tokens: roundUsd(cacheReadCost),
    cache_creation_input_tokens: roundUsd(cacheWriteCost),
    total: totalCost,
  },
  metadata: {
...
if (spend !== null && lf?.lastTraceId) {
  await submitRealSpendScore(lf.lastTraceId, spend, lf.config);
}
