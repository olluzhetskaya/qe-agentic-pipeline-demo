...
Rank candidates from `.agents/artifacts/explore-*.yaml` (live scrape) or
`.agents/artifacts/harvest-*.yaml` (legacy POM). Skip any row whose
`rejected` is not `null`. Do not invent a locator that neither the page
nor the legacy POM showed. Put the chosen locator only in a Page Object
(`pom-builder`). Never paste a scraped string into `tests/`.
