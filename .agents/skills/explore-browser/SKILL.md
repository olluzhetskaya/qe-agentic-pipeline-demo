...
Stage 1 support skill. The coordinator applies it inside `test-generation`,
...
Skip the walk when `src/pages/` already covers the flow and no locator is
missing. Do not invent locators if the session cannot open. Stop instead.
...
only under `src/pages/`. Specs never take scraped strings.
...
4. Hand the scrape to `locator-strategy`, then patch `src/pages/` via `pom-builder`
5. Close the session
