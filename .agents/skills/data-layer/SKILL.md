...
| `wiki/` | Entity definitions the fixtures must match |
...
1. Read `wiki/` for the entity and the rule the case exercises.
...
When wiki introduces an entity that `src/data/` does not model yet:
...
`wiki/` rather than assuming values. The data gate checks schema and placement,
